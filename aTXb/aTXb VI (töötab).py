...
def lihtsusta_väikest_avaldist(avaldis):
    ...
    if B==0:#kui b=0 siis väärtus on a?
        X_miinus_1=a
    return(X_miinus_1)
def arvuta_3liikmelise_avaldise_väärtus(väike_avaldis):
    if "T" in str(väike_avaldis):
...
while 1==1:
    ...
    print(avaldis)
    i=0
    while "T" in str(avaldis):#kuni avaldise asemel pole arvväärtust
        i+=1
        if "T" in str(arvuta_3liikmelise_avaldise_väärtus(leia_sulgude_siseseim_avaldis(avaldis))):
            print(leia_avaldisest_a_b_X(leia_sulgude_siseseim_avaldis(avaldis))[0],leia_avaldisest_a_b_X(leia_sulgude_siseseim_avaldis(avaldis))[2],"a,b")
            if leia_avaldisest_a_b_X(leia_sulgude_siseseim_avaldis(avaldis))[0]=="2.0"==leia_avaldisest_a_b_X(leia_sulgude_siseseim_avaldis(avaldis))[2]:#kui a ja b on kahed ,siis väikese avaldise vastus on 4
                seim=4
            else:
